 ...
 */
inline fun <reified EC, BC, A> Kodein.Builder.scoped(scope: Scope<EC, BC, A>): Kodein.BindBuilder.WithScope<EC, BC, A> = Kodein.BindBuilder.WithScope.Impl(generic(), scope)
 ...
 */
inline fun <EC, BC, reified T: Any> Kodein.BindBuilder.WithScope<EC, BC, Unit>.singleton(ref: RefMaker? = null, sync: Boolean = true, noinline creator: NoArgSimpleBindingKodein<BC>.() -> T) = Singleton(scope, contextType, generic(), ref, sync, creator)
 ...
 */
inline fun <EC, BC, reified A, reified T: Any> Kodein.BindBuilder.WithScope<EC, BC, A>.multiton(ref: RefMaker? = null, sync: Boolean = true, noinline creator: SimpleBindingKodein<BC>.(A) -> T) = Multiton<EC, BC, A, T>(scope, contextType, generic(), generic(), ref, sync, creator)
