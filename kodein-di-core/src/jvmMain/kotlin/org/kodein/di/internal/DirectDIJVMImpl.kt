...
internal actual class DirectDIImpl actual constructor(container: DIContainer, context: DIContext<*>) : DirectDIBaseImpl(container, context), DirectDI {
    ...
    override fun <T : Any> AllProviders(type: TypeToken<T>, tag: Any?): List<() -> T> = container.allProviders(DI.Key(context.anyType, UnitToken, type, tag), context.value)
    ...
    override fun <T : Any> AllInstances(type: TypeToken<T>, tag: Any?): List<T> = container.allProviders(DI.Key(context.anyType, UnitToken, type, tag), context.value).map { it.invoke() }
