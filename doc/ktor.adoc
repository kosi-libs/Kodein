...
You can use Kodein as-is in your Ktor project, but you can level-up your game by using the libraries `kodein-di-ktor-server-jvm` or `kodein-di-ktor-server-controller-jvm`.

NOTE: Kodein does work on Ktor as-is.
      The `kodein-di-ktor-server-jvm` / `kodein-di-ktor-server-controller-jvm` extensions add multiple ktor-specific utilities to Kodein. +
...
----
+
IMPORTANT: either `kodein-di-generic-jvm` or `kodein-di-erased` *must* be declared in addition to the `kodein-di-ktor-server-jvm` package.
+
...
IMPORTANT: either `kodein-di-generic-jvm` or `kodein-di-erased` *must* be declared in addition to the `kodein-di-ktor-server-controller-jvm` package.

NOTE:  the `kodein-di-ktor-server-controller-jvm` already have  the `kodein-di-ktor-server-jvm` as transitive dependency, so you don't need to declare both.
