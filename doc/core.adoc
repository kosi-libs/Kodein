...
[source,groovy,subs="attributes"]
----
compile 'org.kodein:kodein-js:{version}'
----
...
This mechanism is called an Injector.

[source, kotlin]
.Example: using an injector.
----
val injector = KodeinInjector()
val dice: Dice by kodein.on(injector = injector).instance()
/*...*/
injector.inject() <1>
----
<1> Retrieval happens now.

You can, of course, assign multiple properties to the same injector.
You can also create a Kodein object that has a given injector by default:

[source, kotlin]
.Example: creating a Kodein object with an injector.
----
val injector = KodeinInjector()
val injectKodein = kodein.on(injector = injector)
val dice: Dice by injectKodein.instance()
/*...*/
injector.inject()
----

TIP: The `injector` argument should always be named.

NOTE: An injector allows you to "force" retrieval.
...
==== Class global injector

If you want to have all dependency properties retrieved at once, you can use a class global injector.
Simply override the `kodeinInjector` property:

[source, kotlin]
.Example: a KodeinAware class with an injector
----
class MyManager(override val kodein: Kodein) : KodeinAware {
