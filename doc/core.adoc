...
Kodein offers two repositories: one for gradle 4.7 *exactly*, and one that uses the latest version of Gradle.
...
Add the LGM (Latest Gradle Metadata) repository:
...
buildscript {
    repositories {
        maven {
            url "https://dl.bintray.com/kodein-framework/Kodein-DI-LGM"
        }
    }
}
----
...
kotlin {
    sourceSets {
        commonMain {
            dependencies {
                implementation "org.kodein.di:kodein-di-erased:{version}-LGM"
            }
        }
    }
}
...
NOTE: Using LGM has no consequences whatsoever regarding JVM & JS platforms.
