...
Thus, it appears to the developer that there is no differences between platforms, you no longer have to choose between `erased` or `generic` dependencies.
...
To improve performance, you can use the `erased` JVM _Kodein-DI_ version, which is faster, but do suffer from type erasure!
