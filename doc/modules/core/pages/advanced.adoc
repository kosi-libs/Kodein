...
Because _Kodein_ does not depend on `kotlin-reflect`, it uses java `Type` objects that do not contain nullability information.
...
When it detects a recursive dependency, _Kodein_ will throw a `DI.DependencyLoopException`.
...
When you try to inject / retrieve a dependency that does not exists, _Kodein_ will throw a `DI.NotFoundException`.
