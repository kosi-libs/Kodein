...
[NOTE]
====
_Kodein_ provides only 1 scope by default, but:
...
- By scopes that explicitely *support* that feature (not all scopes do, all scopes provided by the _Kodein_ Framework do *except WeakContextScope*).
...
_Kodein_ provides the `WeakContextScope` scope.
