...
----
val di = DI {
    bind<DataSource>() with scopedSingleton(sync = false) { SqliteDS.open("path/to/file") }
}
----
...
----
val di = DI {
    bind<Map>() with scopedSingleton(ref = softReference) { WorldMap() } // <1>
    bind<Client>() with scopedSingleton(ref = weakReference) { id -> clientFromDB(id) } // <2>
}
----
...
----
val di = DI {
    bind<Cache>() with scopedSingleton(ref = threadLocal) { LRUCache(16 * 1024) }
}
----
...
----
val di = DI {
    bind<Dice>() with scopedSingleton { Dice(instance(), instance(tag = "max")) } // <1>
...
----
val di = DI {
    bind<DataSource>() with scopedSingleton { MySQLDataSource() }
...
----
val di = DI {
    bind<Manager>() with scopedSingleton { ManagerImpl(di) } // <1>
}
----
