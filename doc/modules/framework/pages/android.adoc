...
The android module provides a number of xref:ROOT:core.adoc#context_translators[context translators].
...
NOTE: This scope *IS* compatible with `ScopeCloseable`: xref:ROOT:core.adoc#scope-closeable[see documentation].
...
[NOTE]
====
You can define the way the parent kodein is extended by defining the xref:ROOT:core.adoc#overridden_access_from_parent[copy mode]:
