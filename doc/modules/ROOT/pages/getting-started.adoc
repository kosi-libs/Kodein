...
val kodein = DI {
    ...
    bind<Database>() with scopedSingleton { SQLiteDatabase() }
}
----
...
----
val kodein = DI {
    bind<Database>(tag = "local") with scopedSingleton { SQLiteDatabase() }
...
----
val di = DI {
    bind<Presenter>() with scopedSingleton { Presenter(instance(), instance()) }
}
----
...
----
val di = DI {
    bind<Presenter>() with scopedSingleton { Presenter(di) }
}
----
