...
[IMPORTANT]
====
Kodein {version} is built upon and compatible with Kotlin {kotlin}.
...
- _Kodein_ is already pretty optimized ;)
...
//Because the `erased` version does not use that trix, handling generic types in _Kodein_ are a lot more complex to use.
//
//[options="header"]
//|=======
...
NOTE: Even if by default _Kodein_ uses a `generic` type system, you still can use the `erased` one manually.
//(see *TODO*)
