...
If you want all dependencies to be retrieved at `onCreate`, you can very easily use an injector:

[source, kotlin]
.Example: using an injector in a KodeinAware Android Activity
----
class MyActivity : Activity(), KodeinAware {
    override val kodein by closestKodein()
