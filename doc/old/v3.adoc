= KOtlin DEpendency INjection V3
...
CAUTION: Every-thing that is described here is *a lot less performant* than using classic kodein injection methods.
         ...
         Kitties *will* die painfully if you do!
...
* `generic<String>().type` is the String's java `Class`.
* `generic<List<String>>.type` is a `ParameterizedType` describing exactly a `List<String>`.
