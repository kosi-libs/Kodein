...
This leads to a problem, if you are using generic types in the DI container, because the compiler erases generic type information when using Kodein-DI API.
...
Kodein-DI offers two different flavours: `kodein-generic` & `kodein-erased`.
