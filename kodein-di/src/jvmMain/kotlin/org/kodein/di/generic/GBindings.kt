 ...
 */
inline fun <reified C> DI.Builder.scoped(scope: Scope<C>): DI.BindBuilder.WithScope<C> = DI.BindBuilder.WithScope.Impl(generic(), scope)
 ...
 */
inline fun <reified C> DI.Builder.contexted(): DI.BindBuilder.WithContext<C> = DI.BindBuilder.WithContext.Impl(generic())
 ...
 */
inline fun <C, reified A, reified T: Any> DI.BindBuilder.WithContext<C>.factory(noinline creator: BindingDI<C>.(A) -> T) = Factory<C, A, T>(contextType, generic(), generic(), creator)
 ...
 */
inline fun <C, reified T: Any> DI.BindBuilder.WithContext<C>.provider(noinline creator: NoArgBindingDI<C>.() -> T) = Provider(contextType, generic(), creator)
 ...
 */
inline fun <C, reified T: Any> DI.BindBuilder.WithScope<C>.singleton(ref: RefMaker? = null, sync: Boolean = true, noinline creator: NoArgSimpleBindingDI<C>.() -> T) = Singleton(scope, contextType, generic(), ref, sync, creator)
 ...
 */
inline fun <C, reified A, reified T: Any> DI.BindBuilder.WithScope<C>.multiton(ref: RefMaker? = null, sync: Boolean = true, noinline creator: SimpleBindingDI<C>.(A) -> T) = Multiton<C, A, T>(scope, contextType, generic(), generic(), ref, sync, creator)
