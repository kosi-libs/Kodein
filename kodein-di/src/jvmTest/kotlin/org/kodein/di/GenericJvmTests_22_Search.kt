...
class GenericJvmTests_22_Search {
    ...
    fun test_00_SearchTagged() {
        ...
        val values = bindings.map { (key, _) ->
            ...
            kodein.container.factory(key as DI.Key<Any, Any?, Any>, DIContext.Any).invoke(Unit)
        }
    ...
    fun test_01_SearchArgument() {
        ...
        val values = bindings.map { (key, _) ->
            @Suppress("UNCHECKED_CAST")
            kodein.container.factory(key as DI.Key<Any, Unit, Any>, DIContext.Any).invoke(Unit)
        }
