...
If you are migrating from Kodein 2.8, you can read the https://github.com/SalomonBrys/Kodein/blob/master/MIGRATION-2-3.md[migration document].
...
[WARNING]
====
Using a mutable `ConfigurableKodein` can lead to *very bad spaghetti code*.
Therefore, using a mutable `ConfigurableKodein` *IS discouraged*, unless you truly need it, know what you're doing, and see no other way.
====

A mutable `ConfigurableKodein` can be configured even _after first retrieval_ by using the `mutate*` functions.
...
fun test() {
    ...
    val ds: DataSource = kodein.instance() <1>

    kodein.mutateAddModule(anotherModule)
}
----
<1> Passed this line, you need to use the `mutate*` functions.

You can also use reset to put a mutable `ConfigurableKodein` back into a configurable state.

[source, kotlin]
.Example: reseting a mutable ConfigurableKodein
----
fun test() {
    val kodein = ConfigurableKodein(mutable = true)
    ...
    kodein.mutateReset(true) <1>
    kodein.addModule(anotherModule)
}
----
<1> `true` to keep current bindings, false to reset all bindings.
...
----
val kodein = Kodein {
    bind<Logger>() with contextSingleton { LogManager.getNamedLogger(it.localClassName) } // <1>
}
----
...
----
val logger = kodein.with(context).instance()
val sameLogger = kodein.with(context).instance() // this will be the same object as logger
val otherLogger = kodein.with(otherContext).instance() // this will be a different object than logger
----
...
----
val kodein = Kodein {
    bind<Logger>() with serviceSingleton { LogManager.getNamedLogger(it.localClassName) } // <1>
}
----
...
----
val logger = kodein.with(service).instance()
----
...
----
val kodein = Kodein {
    bind<Logger>() with broadcastReceiverSingleton { LogManager.getNamedLogger(it.localClassName) } // <1>
}
----
...
----
val logger = kodein.with(broadcastReceiver).instance()
----
...
----
val kodein = Kodein {
    bind<Logger>() with fragmentSingleton { LogManager.getNamedLogger(it.localClassName) } // <1>
}
----
...
----
val kodein = Kodein {
    bind<Logger>() with supportFragmentSingleton { LogManager.getNamedLogger(it.localClassName) } // <1>
}
----
...
----
val logger = kodein.with(fragment).instance()
----

[source, kotlin]
.Example: retrieving a support fragment scoped singleton
----
val logger = kodein.with(supportFragment).instance()
----
...
The API reference can be found https://github.com/SalomonBrys/Kodein/blob/master/dokka/out/doc/index.md[here]!
...
If you feel like enhancing this document, you can propose a pull request that modifies https://github.com/SalomonBrys/Kodein/blob/master/README3.adoc[README3.adoc].
