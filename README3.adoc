...
class Controller() {
    ...
    fun whenReady(kodein: Kodein) = injector.inject(kodein) // <3>
}
----
<1> Creating an injector
<2> Injecting properties at construction.
<3> Injecting all properties created by the injector.
...
1. Declare the dependency bindings in the Android `Application`, having it implements `KodeinAware`.
+
[source, kotlin]
...
2. In your Activities, Fragments, and other context aware android classes, retrieve dependencies!
...
[source, kotlin]
.Retrieving dependencies with `appKodein`
----
class MyActivity : Activity() {
    val kodein = LazyKodein(appKodein)

    val diceProvider: () -> Dice by kodein.lazyProvider() // <1>
...
[source, kotlin]
.Retrieving dependencies with an injector
----
class MyActivity : Activity() {
