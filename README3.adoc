...
`KodeinActivity`, `KodeinFragmentActivity`, and `KodeinAppCompatActivity` will internally initialize their injector before they call `super.onCreate` (see https://github.com/SalomonBrys/Kodein/issues/50[the issue that necessitates this]).
...
When using `ActivityInjector`, `FragmentActivityInjector`, or `AppCompatActivityInjector` it is suggested to call `initializeInjector` before `super.onCreate` (see https://github.com/SalomonBrys/Kodein/issues/50[the issue that necessitates this]).
