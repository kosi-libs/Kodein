...
[source,kotlin]
.Initialization of a Kodein variable
----
val kodein = Kodein {
	/* Bindings */
}
...
[source,kotlin]
.Creates a new `Dice` each time the you need one, according to an `Int` representing the number of sides
----
val kodein = Kodein {
...
[source,kotlin]
.Creates a new 6 sided `Dice` entry each time you need one
----
val kodein = Kodein {
...
[source,kotlin]
.Creates a `DataSource` singleton that will be initialized on first access.
----
val kodein = Kodein {
    bind<DataSource>() with singleton { SqliteDS.open("path/to/file") }
}
...
[source,kotlin]
.Creates a `DataSource` singleton that will be initialized as soon as the binding block ends.
----
val kodein = Kodein {
...
[source,kotlin]
.Creates a `Cache` object that will exist once per thread.
----
val kodein = Kodein {
...
[source,kotlin]
.A `DataSource` binding to an already existing instance.
----
val kodein = Kodein {
...
[source, kotlin]
.A class that needs transitive dependencies
----
...
[source, kotlin]
.Bindings of `Dice` and of its transitive dependencies
----
val kodein = Kodein {
...
[source, kotlin]
.Extends an already existing Kodein instance
----
...
[source, kotlin]
.Binds twice the same type, the second time explitly specifying an override
----
val kodein = Kodein {
    bind<API>() with singleton { APIImpl() }
    /* ... */
...
[source, kotlin]
.Imports a module and giving it the right to override existing bindings.
----
val kodein = Kodein {
    /* ... */
...
[source, kotlin]
.Declaring a module in which each binding may or may not override existing bindings.
----
...
[source, kotlin]
.Retrieving bindings that may not have been bound
----
...
[source, kotlin]
.Retrieving lazy curried factory properties
----
...
[source, kotlin]
.Retrieving lazy curried factory properties with lazy parameters
----
...
[source, kotlin]
.Retrieving properties via an injector
----
class Controller() {
...
[source, kotlin]
.Creating curried factory properties
----
...
[source, kotlin]
.Retrieving a logger in a KodeinAware class
----
class MyManager(override val kodein: Kodein): KodeinAware {
...
[source, kotlin]
.Adding a module inside the global Kodein
----
...
+
[source, kotlin]
.An Android `Application` class that implements `KodeinAware`
----
...
[source, kotlin]
.Retrieving dependencies with `LazyKodein` in Android
----
...
[source, kotlin]
.Retrieving dependencies with an injector in Android
----
class MyActivity : Activity() {
...
[source, kotlin]
.Retrieving dependencies with `LazyKodeinAware` in Android
----
...
[source, kotlin]
.Retrieving dependencies with `KodeinInjected` in Android
----
...
[source, kotlin]
.Using kodein to retrieve a `LayoutInflater`
----
class MyActivity : Activity(), LazyKodeinAware {
    override val kodein = LazyKodein(appKodein)
...
[source, kotlin]
.Retrieving an activity scoped singleton
----
...
[source, kotlin]
.Retrieving an auto activity scoped singleton
----
...
[source, kotlin]
.Registering a callback at binding time
----
...
[source, kotlin]
.Defining a scope and binding a singleton inside it
----
...
[source, kotlin]
.Defining an scope and binding a singleton inside it
----
...
[source, kotlin]
.Using an auto scope, without knowing it
----
