...
class MyApplication : Application(), KodeinAware {
	override val kodein by Kodein.lazy {
	    ...
	    bind<String>() with instance("MyApplication")
	}
}
----
...
class MyActivity : KodeinActivity() { // <1>
    private val logTag: String by instance() // <2>
    private val app: Application by injector.instance() // <3>

    override fun provideOverridingBindings() : Kodein.Builder.() -> Unit = { // <4>
        bind<MyActivity>() with instance(this@MyActivity)
        bind<String>(overrides = true) with instance("MyActivity")
    }
...
<4> `provideOverridingBindings` allows us to override bindings specified higher up in the dependency tree (for example, we override the `String` binding defined in `MyApplication`)
...
Kodein also provides a set of interfaces that provide the same functionality as the inheritance based method. The only difference is that the injector lifecycle must be managed. In almost every case, this can be accomplished by simple calling `initializeInjector` immediately after `onCreate` and `destroyInjector` immediately after `onDestroy`.
...
class MyFragment : CustomFragment(), FragmentInjector { // <1>
    ...
    private val logTag: String by instance() // <3>
    private val app: Application by injector.instance() // <4>

    override fun provideOverridingBindings() : Kodein.Builder.() -> Unit = { // <5>
        bind<MyFragment>() with instance(this@MyFragment)
        bind<String>(overrides = true) with instance("MyFragment")
    }
...
----
<1> Because we extends `CustomFragment` we can't extend `KodeinFragment` so instead we implement `FragmentInjector`
...
<5> `provideOverridingBindings` allows us to override bindings specified higher up in the dependency tree (for example, we override the `String` binding defined in `MyApplication`)
