 ...
 */
operator fun BindingsMap.contains(type: Type): Boolean = any { it.key.bind.type == type }
 ...
 */
fun BindingsMap.factoryArgumentTypes(bind: Kodein.Bind): List<Type> = filter { it.key.bind == bind } .map { it.key.argType }
 ...
 *         but there may be multiple [Kodein.Key] for the same [Kodein.Bind].
 */
fun BindingsMap.types(tag: Any?): List<Type> = filter { it.key.bind.tag == tag } .map { it.key.bind.type } .distinct()
