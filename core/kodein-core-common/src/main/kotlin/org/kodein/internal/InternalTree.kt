...
internal class InternalTree(
    ...
    private val _cache: MutableMap<Kodein.Key<*, *, *>, List<KodeinDefinedBinding>> = HashMap()
...
//        val list = it8.toList()
    ...
    fun <C, A, T : Any> find(key: Kodein.Key<C, A, T>, overrideLevel: Int): Result<C, A, T> {
        ...
        _cache[key]?.let { return Result(found = it.getOrNull(overrideLevel)?.binding as KodeinBinding<C, A, T>?) }

        if (key.contextType != AnyToken) {
            val anyContextKey = key.copy(contextType = AnyToken)
            _cache[anyContextKey]?.let {
                _cache[key] = it
                return Result(found = it.getOrNull(overrideLevel)?.binding as KodeinBinding<C, A, T>?)
            }
        }
        ...
        return Result(found = queue.getOrNull(overrideLevel)?.binding as KodeinBinding<C, A, T>?)
    }

    @Suppress("UNCHECKED_CAST")
    operator fun <C, A, T: Any> get(key: Kodein.Key<C, A, T>) = _cache[key]
