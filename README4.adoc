...
image:https://img.shields.io/badge/Kotlin-1.1-blue.svg[alt="Kotlin 1.1.2-4", link="http://kotlinlang.org"]
...
Note that because of the package difference, shared JVM / JS code need to either use `kodein-erased` or the uppercase functions.
Read <<jvm-performance>> to learn more.

==== Primitives

Currently, Kotlin JS does not support reflection over primitives. +
This means that you cannot bind `String` or `Int` instances.

The use of the `constant(tag) with value` syntax is therefore discouraged since this syntax is mainly used with primitives.

To circumvent this, you need to encapsulate any primitive you need to bind into a data class.

The issue has been https://youtrack.jetbrains.com/issue/KT-18172[reported].

Note that this means that you cannot use primitives in factories parameter either :(
...
----
<1> Because `sqlDS` is of type `SQLiteDataSource` and not `DataSource`, this will fail in JS

To have the retrieval working in JS, you need to pass an argument of the exact type, which you can have by casting:
...
The Configurable Kodein extension is available for JS, without any difference from the JVM version.
...
The uppercase functions needs `TypeToken` parameters that define the type being bound / retrieved and maybe the factory's argument. +
